...
var showWordButtons = function() {
    ...
    var pastWordsListEl = document.createElement("div");
    ...
    for (var i = 0; i < wordList.length; i++) {
        //create list item for the list
        var wordListItem = document.createElement("span");
        // sets the id to li-[word]
        wordListItem.setAttribute("id", "span-" + wordList[i].word);
        // creates a button element
        var wordBtnEl = document.createElement("button");
        // sets the id to btn-[word]
        wordBtnEl.setAttribute("id", "btn-" + wordList[i].word);
        // adds the class
        wordBtnEl.className = "word-btn btn is-rounded is-size-5-mobile is-size-2-tablet";
        // sets the button to show the word
        wordBtnEl.textContent = wordList[i].word;
        // appends the button to the list item element
        wordListItem.appendChild(wordBtnEl);
        // appends the list item to the list element
        pastWordsListEl.appendChild(wordListItem);
    }
...
var showDef = function(wordObject) {
    ...
    $('.past-word-list').append(defDivEl);
};
